...
This package contains x sample datasets, listed below:

1. `CGTmock`: A dataframe of lineup choices for 3 independent lineups
2. `line73` : Lineup data from Doob & Kirshenbaum (1973) 
3. `mockdata` : Real lineup data from a study conducted at Toulouse??
4. `mickwick` :Confidence and accuracy data for lineup choices. Sample data to be passed to ROC function `make_roc()`
5. Other data
6. Image data for face_sim

All sample datasets can be loaded by calling `data(sample_data)`. Further details can be found in the 
...
* Without passing any arguments, call `face_sim()`:
    ...
    * A dialog box will appear, through which you may access your OS's file explorer. Navigate to the folder in which the images are stored, select all, and open.
    ...
    The images used in this vignette can be found in the data subdirectory for this 
            package. --- HAVE TO EDIT THIS. INCLUDE AS A .RDA FILE, REMAKE WITH MAGICK
