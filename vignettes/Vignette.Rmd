...
This package contains x datasets, listed below:

1. CGTmock
  * A dataframe of lineup choices for 3 trials (3 columns x 133 observations)
2. 
...
Briefly, the argument 'conf' allows you to specify alpha, and 'type' specifies the type of confidence interval to be returned. If type = "all", the function will return normal theory, basic, percentile and bias-corrected confidence intervals.
...
lineuprops.ci
```
The object `lineuprops.ci` is a dataframe of 2 columns and k rows, in which columns 1 & 2 contain lower and upper confidence intervals, respectively, and the number of rows = nominal size. 
...
* _Compute bootstrapped estimates (Malpass's adjusted):_
    ...
    To do this, one must first generate a bootstrap sample using the gen_boot_samples function, the `gen_esize_m function`, and the `gen_esize_m_ci` functions.  Their usage is shown below.
...
This function allows you to compute and plot an ROC curve for data from an eyewitness experiment, where accuracy is recorded for target present and target
